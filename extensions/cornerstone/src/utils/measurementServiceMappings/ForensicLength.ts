...
const ForensicLength = {
  ...
  toMeasurement: (
    ...
    if (!metadata || !data) {
      console.warn('ForensicLength tool: Missing metadata or data');
      return null;
    }
...
function _getReport(mappedAnnotations, points, FrameOfReferenceUID, customizationService) {
  ...
  values.push('Cornerstone:ForensicLength');

  mappedAnnotations.forEach(annotation => {
    const { length, unit } = annotation;
    columns.push(`ForensicLength`);
    values.push(length);
    columns.push('Unit');
    values.push(unit);
  });
...
function getDisplayText(mappedAnnotations, displaySet, customizationService) {
  ...
  const roundedForensicLength = utils.roundNumber(length, 2);
  displayText.push(`${roundedForensicLength} ${unit} (S: ${SeriesNumber}${instanceText}${frameText})`);

  return displayText;
}

export default ForensicLength;
